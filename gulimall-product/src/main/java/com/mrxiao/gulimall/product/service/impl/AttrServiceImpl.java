...
import com.mrxiao.gulimall.product.dao.AttrDao;
import com.mrxiao.gulimall.product.dao.AttrGroupDao;
...
public class AttrServiceImpl extends ServiceImpl<AttrDao, AttrEntity> implements AttrService {
    ...
    public void saveAttr(AttrVo attr) {
        ...
        System.out.println("★★★新增商品属性attr"+attr);
        System.out.println("★★★新增商品属性attrEntity"+attr);
        //1、保存基本数据
    ...
    public List<AttrEntity> getAttrRelation(Long attrgroupId) {
        ...
        List<Long> attrIds = relationList.parallelStream().map(AttrAttrgroupRelationEntity::getAttrId).collect(Collectors.toList());
    ...
    public void deleteRelation(AttrGroupRelationVo[] vos) {
        ...
        List<AttrAttrgroupRelationEntity> entities = Arrays.asList(vos).parallelStream().map(item -> {
...
//    }
    ...
    public PageUtils getNoRelationAttr(Map<String, Object> params, Long attrgroupId) {
        ...
        System.out.println("★★获取当前分组没有关联的所有属性params"+" attrgroupId:"+attrgroupId);
        AttrGroupEntity attrGroupEntity = attrGroupDao.selectById(attrgroupId);
        Long catelogId = attrGroupEntity.getCatelogId();
        //2、当前分组只能关联别的分组没有引用的属性
        //2.1)、当前分类下的其他分组
        List<AttrGroupEntity> group = attrGroupDao.selectList(new QueryWrapper<AttrGroupEntity>().eq("catelog_id", catelogId));
        System.out.println("★★获取当前分组没有关联的所有属性group"+group);
        List<Long> collect = group.stream().map(item -> item.getAttrGroupId()).collect(Collectors.toList());

        //2.2)、这些分组关联的属性
        List<AttrAttrgroupRelationEntity> groupId = relationDao.selectList(new QueryWrapper<AttrAttrgroupRelationEntity>().in("attr_group_id", collect));
        System.out.println("★★获取当前分组没有关联的所有属性groupId"+groupId);
    ...
    public List<Long> selectSearchAttrIds(List<Long> attrIds) {
        /**
         * SELECT * from pms_attr WHERE attr_id IN(3) AND search_type=1
         */
        List<Long> attrId=  baseMapper.selectSearchAttrIds(attrIds);
        return attrId;
    }
